...
APP_SUBTITLE = "Web version based purely on the newer standalone GIWAXS code · Developed by Richard Li"
...
def load_qimage_npz(file_obj, flip_qx: bool):
    data = np.load(io.BytesIO(file_obj.getvalue()))
...
def process_files(preset, sample_files, bg_files, settings, filter_text):
                ...
                if settings["subtract_bg"]:
                    ...
                    if bg_path is None:
                        ...
                        if settings["skip_missing_bg"]:
                            ...
                            continue
                        raise ValueError(msg)
                out_entries.append(plot_qimage_one(path, settings, bg_file=bg_path, sample_theta=sample_theta))
            elif preset == "Qphi":
                out_entries.append(plot_qphi_one(path, settings))
            elif preset == "Azimuthal":
                out_entries.extend(plot_azimuthal_one(path, settings))
            elif preset == "CirAvg":
                out_entries.append(plot_ciravg_one(path, settings))
            else:
    ...
    with zipfile.ZipFile(zip_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for name, content in out_entries:
            zf.writestr(name, content)
        zf.writestr("export_log.txt", "\n".join(logs))
    zip_buffer.seek(0)
    return zip_buffer.getvalue(), exported, skipped, logs
...
def main():
    ...
    with export_col:
        if st.button("Build export ZIP", type="primary", use_container_width=True):
            if errors:
                ...
                try:
                    zip_bytes, exported, skipped, logs = process_files(preset, sample_files, bg_files, settings, filter_text)
                    st.session_state.giwaxs_zip = zip_bytes
                    st.session_state.giwaxs_exported = exported
                    st.session_state.giwaxs_skipped = skipped
                    st.session_state.giwaxs_logs = logs[-50:]
                    st.success(f"Export complete. Exported {exported}, skipped {skipped}.")
                except Exception as e:
                    st.error(str(e))

    if "giwaxs_zip" in st.session_state:
        st.download_button(
            "Download export ZIP",
            data=st.session_state.giwaxs_zip,
    ...
    with st.expander("Notes"):
        st.markdown(
            """
- This web app is based purely on the newer standalone GIWAXS code, not the older PYRL GIWAXS section.
...
- **CirAvg** reads `.csv` files and prefers files containing `cir` or `avg` in the filename, matching the standalone script behavior.
            """
        )
